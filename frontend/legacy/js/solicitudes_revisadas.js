// js/solicitudes_revisadas.js
// Página para ver solicitudes finalizadas (aprobadas/rechazadas)
// - Trabajador: ve sus propias solicitudes finalizadas
// - Jefe: ve las suyas y las de sus trabajadores a cargo
// - Administrador: ve todas las solicitudes finalizadas

let authToken = localStorage.getItem("authToken");
let currentUser = null;

// Datos
let solicitudesRevisadas = [];
...
/* ========== CARGAR SOLICITUDES REVISADAS ========== */

async function cargarSolicitudesRevisadas() {
    try {
        // Cargar requests (permisos y notificaciones) finalizadas
        const resReq = await fetch(API_BASE + "/api/requests?finished=1", {
            headers: getHeaders()
        });
        const dataReq = await resReq.json();
        const requests = dataReq.requests || [];

        // Cargar rendiciones finalizadas
        let rendiciones = [];
        try {
            const resRend = await fetch(API_BASE + "/api/rendiciones?finished=1", {
                headers: getHeaders()
            });
            if (resRend.ok) {
        ...
        // Filtrar solo finalizadas de requests
        // Los Abonos siempre se consideran finalizados
        const requestsFinalizadas = requests.filter(r =>
            r.type === "Abono" || estadosFinalizados.includes(r.status)
        );

        // Filtrar solo finalizadas de rendiciones
        const rendicionesFinalizadas = rendiciones.filter(r =>
            estadosFinalizados.includes(r.status)
        );

        // Combinar y normalizar
        solicitudesRevisadas = [
            ...requestsFinalizadas.map(r => ({
                id: r.id,
        ...
        // Actualizar total
        paginacion.total = solicitudesRevisadas.length;
...
function renderTabla() {
    const tbody = document.getElementById("tbody-revisadas");
    const search = busqueda.toLowerCase();

    // Filtrar por búsqueda
    let filtrados = solicitudesRevisadas.filter(s => {
    ...
    if (filtroEstado) {
        filtrados = filtrados.filter(s => {
            const estadoNormalizado = normalizarEstado(s.status);
    ...
    // Actualizar info
    const infoEl = document.getElementById("info-revisadas");
    if (totalFiltrado === 0) {
...
function renderPaginacion(total) {
    const container = document.getElementById("pagination-revisadas");
...
function setupOrdenamiento() {
    document.querySelectorAll(".rev-table th.sortable").forEach(th => {
        th.addEventListener("click", () => {
            ...
            document.querySelectorAll(".rev-table th.sortable").forEach(t => {
...
function setupBusquedaYFiltros() {
    // Búsqueda
    document.getElementById("search-revisadas").addEventListener("input", (e) => {
    ...
    // Límite por página
    document.getElementById("limit-revisadas").addEventListener("change", (e) => {
...
function normalizarEstado(status) {
    // Normaliza estados para el filtro
    if (status.includes("aprobado") || status === "informativa") {
        return "aprobada";
    }
    if (status.includes("rechazado") || status === "rechazada") {
        return "rechazada";
    }
    return status;
}
...
(async function init() {
    ...
    await cargarSolicitudesRevisadas();
})();
